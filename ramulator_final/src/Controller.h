...
public:
    ...
    Controller(const Config& configs, DRAM<T>* channel) :
        ...
        //TVP cmd_trace_files(channel->children.size())
        cmd_trace_files(8)
    {
        ...
        if (record_cmd_trace){
            ...
            //TVP for (unsigned int i = 0; i < channel->children.size(); i++)
...
private:
    ...
    void issue_cmd(typename T::Command cmd, const vector<int>& addr_vec)
    {
        ...
        if (record_cmd_trace){
            // select rank
            // cout << "addr_vec[0]: " << addr_vec[0] << endl; 
            auto& file = cmd_trace_files[addr_vec[0]];

            // cout << "cmd_trace_files: " << cmd_trace_files.size() << endl; 
            //cout << "cmd_trace_files: " << cmd_trace_files[addr_vec[0]] << endl; 
            // cout << "before"<< endl;
            string& cmd_name = channel->spec->command_name[int(cmd)];
            // cout << cmd_name << endl;
            // cout << count << endl;
            // count++;
            // cout << "size: "<<addr_vec.size() << endl;
            file<<clk<<','<<cmd_name;
            // TODO bad coding here
            //cout << int(T::Level::Channel) << endl;
            int channel_id = addr_vec[int(T::Level::Channel)];
            //int channel_id = addr_vec[0];
            ...
            // }
                ...
                int col_id = addr_vec[int(T::Level::Column)];  
                // file<<','<<channel_id;
                // file<<','<<rank_id;
                // file<<','<<bank_id;
                if (channel->spec->standard_name == "DDR4" || channel->spec->standard_name == "HBM" || channel->spec->standard_name == "GDDR5")
                ...
                if (cmd_name == "ACT")
                    file<<','<<row_id<<endl;
                else if(cmd_name=="RD" || cmd_name=="RDA" || cmd_name=="WR" || cmd_name=="WRA"){

                  //  file<<','<<row_id;
                    file<<','<<col_id<<endl;
                   // file<<','<<addr_vec[col_id]<<endl;
                }
                else
                    file<<endl;                                
            }
